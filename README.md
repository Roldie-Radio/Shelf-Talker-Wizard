...
- **THC/CBD** (a Product Type alongside Wine / Spirits, Bourbon, and Beer) &mdash; for hemp-derived, cannabis-infused ready-to-drink beverages. Picking it adds THC (mg per serving) and CBD (mg per serving) fields, an optional Servings per Container field, and a Lab Tested switch. THC and/or CBD print as a pair of colored potency badges under the description, with a fixed "21+ Only &middot; Keep Out of Reach of Children" compliance line underneath (always printed for this Product Type, not a checkbox) &mdash; Lab Tested adds its own small badge next to that line when checked. There's no external lookup or shared library for this category (no live data source exists for it the way Untappd backs Beer) &mdash; potency figures are typed in by hand. Switching a talker's Product Type away from THC/CBD hides all of this again without deleting anything already on it.
