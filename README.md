...
- **The Beer Bible** (a permanent sidebar rail icon, next to Bourbon Library and The Pairing Atlas) &mdash; a shared, store-wide record of researched beers: Brewery, Location, Style, ABV, IBU, Untappd Rating (with count), Tasting Notes, and SKU, mirroring the fields Beer already pulls from Untappd. Click the rail icon to browse/search every saved beer as a card grid, **Add Beer** to research a new one, or click a card to edit or delete it. This is a first, bare-scaffold cut of the idea &mdash; two things the Bourbon Library above also has that this doesn't yet: cross-register sync (each PC's Beer Bible is its own local copy for now, unlike the Mash Bill Library's Server PC setup) and an Edit Talker recall banner (saving a beer here doesn't yet fill anything back into a talker automatically). A **Check GitHub for New Beers** button on the page can pull in a curated list the same way the Bourbon Library's own GitHub sync does, once one exists &mdash; the bundled seed list ships empty today.
