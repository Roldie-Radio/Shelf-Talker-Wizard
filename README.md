...
- **Bourbon Shelf Talkers** (experimental, off by default &mdash; Settings &rarr; Experimental Features) &mdash; adds Nose / Palate / Finish fields to Wine / Spirits Shelf Talkers, printed under the description, and a Distiller.com source to **Find Tasting Notes** that fills them in automatically. Distiller's scraper hasn't been confirmed against the live site, so always double-check what it finds before printing. Turning the toggle off hides the fields and the source again without deleting anything already on a talker.
