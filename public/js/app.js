(function () {
  ...
  // with - see checkWhatsNew (unseen-only) and onShowWhatsNewRequested
  // (everything) below. Remembered so the "Hide Previous Updates" toggle
  // has something to collapse back to.
  let whatsNewBaseEntries = [];
  let whatsNewExpanded = false;

  // Shared by the automatic launch check and the Electron Help menu's
  // "What's New" item, which just show a different slice of the list
  // (unseen-only vs. everything). Resets the collapse toggle back to
  ...
  // and reopen it from the app-bar button.
